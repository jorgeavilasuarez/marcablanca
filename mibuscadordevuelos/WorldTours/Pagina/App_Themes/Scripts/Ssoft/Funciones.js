    ...
    function InicialzarJBuscador_PreInit() {
        ...
        function Window_Load() {
            ...
            try {
                ...
                TexBox_Calendario_Buscador_Hotel.datepicker({
                    changeMonth: true, changeYear: true, minDate: +2, maxDate: '+1Y', numberOfMonths: 2, dateFormat: FormatoFecha,
...
function SetIdHotel(IdHotel) {
    ...
    $('#iHotel').attr('src', 'Hotel?intIdHotel=' + IdHotel);
    $find('MPEEHotel').show();
    //$object("MPEEHotel").show();
}
...
function SetIdHotelRot(IdHotel) {
    ...
    $('#iHotelRot').attr('src', 'Hotel?intIdHotel=' + IdHotel);
...
function Pagina_Reserva_Hotel_Cargar() {
    /*OBTENEMOS EL CHECKBOX*/
    var chkAcepto_Hotel = $("input[id*='cbAceptar']");
    /*SUSCRIBIMOS EL METODO AL EVENTO CLICK*/
...
function HabilarDeshabilitarHotel(chkAcepto_Hotel) {
    ...
    var btnConfirmar = $("input[id*='btnFinalizar']");
    /*VERIFICAMOS SI EL CHECKBOX EXISTE*/
    if (chkAcepto_Hotel.length) {
        //alert(chkAcepto_Hotel.prop("checked"));
        if (chkAcepto_Hotel.prop("checked")) {
...
function HabilarDeshabilitar(chkAcepto) {
    ...
    /*VERIFICAMOS SI EL CHECKBOX EXISTE*/
    if (chkAcepto.length) {
        //alert(btnConfirmar.attr("disabled"));
...
$(document).ready(function () {
    ...
    $("input[id*='txtNumTarjeta']").validarTarjetaCredito(0);
});

function BorrarTarjeta() {
    $("input[id*='txtNumTarjetaPOL']").removeClass("lblError")
    $("input[id*='txtNumTarjetaPOL']").val("");
    $("#tipoTarjeta").html("");
}
...
function popUp(valor) {
    try {
        $("input[id*='txtFranPOL']").val($(".tarjetasccPOL").val());
        $("input[id*='txtCuoPOL']").val($("[id*='ddlCuotasPOL']").val());
        $("input[id*='txtMPOL']").val($(".MesesccPOL").val());
        $("input[id*='txtAPOL']").val($(".AniosccPOL").val());
    } catch (Ex) { }
    ...
    if (valor == 1) {
        //while (salir) {  
        var rdbFormasPago = $("input:radio[name*='opcionPago']:checked").val();

        if (rdbFormasPago === "TCPOL") {
            if ($("input[id*='txtNumTarjetaPOL']").val() == "" || $("input[id*='txtTitularPOL']").val() == "" ||
                $("input[id*='txtIdentificacionPOL']").val() == "" || $("input[id*='txtDireccionPOL']").val() == "" ||
                $("input[id*='txtCiudadPOL']").val() == "" || $("input[id*='txtTelefonoOficinaPOL']").val() == "" ||
                $("input[id*='txtTelefonoOtroPOL']").val() == "") {
                validacion = false;
            }
            else {
                ...
                var txtCodSeguridad = $("input[id*='txtCodSeguridadPOL']").val();
                if (txtCodSeguridad.length < 3 && franquicia.indexOf("AME") == -1) {
                    $("input[id*='txtCodSeguridadPOL']").val("Muy corto");
                    $("input[id*='txtCodSeguridadPOL']").addClass("lblError");
                    validacion = false;
                    //alert('con segu');
                }
                else {
                    if (txtCodSeguridad.length > 3 && franquicia.indexOf("AME") == -1) {
                        $("input[id*='txtCodSeguridadPOL']").val("Muy largo");
                        $("input[id*='txtCodSeguridadPOL']").addClass("lblError");
                        validacion = false;
                        //alert('con segu');
                    }
                    else {
                        if (txtCodSeguridad.length < 4 && franquicia.indexOf("AME") != -1) {
                            $("input[id*='txtCodSeguridadPOL']").val("Muy corto");
                            $("input[id*='txtCodSeguridadPOL']").addClass("lblError");
                            validacion = false;
                            //alert('con segu');
        ...
        //alert(validacion);
        if (validacion == false) {
            $(".lblErrorTarjetaPOL").text("Por favor diligencie los datos marcados como obligatorios (*)");
